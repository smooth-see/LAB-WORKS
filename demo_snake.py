...
def draw_grid():
    for i in range(HEIGHT//CELL):
        for j in range(WIDTH//CELL):             # рисую сетку  1 толщина контуров 
            pygame.draw.rect(screen, colorGRAY, (i*CELL, j*CELL, CELL, CELL), 1)
def draw_grid_chess():
    color=[colorWHITE, colorGRAY]
    for i in range(HEIGHT // CELL):
        for j in range(WIDTH // CELL):
            pygame.draw.rect(screen, color[(i + j) % 2], (i * CELL, j * CELL, CELL, CELL)) #выресовываем сетку
class Point:
    def __init__(self, x, y):
        self.x=x
        self.y=y
class Snake:
    def __init__(self):  #голова      тулвище         хвост
        self.body = [Point(10, 11), Point(10, 12), Point(10, 13)]
        #начальное движение змейки на право 
        self.dx = 1 
        self.dy = 0

    def move(self): #движение змейки и цикл идет с конца змейки не включаю голову 
        #то есть каждый элемент копирует по следующий и следует за головой 
