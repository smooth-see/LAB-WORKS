...
#создание таблицы
def init_db():
    conn = psycopg2.connect(dbname='snake_game', user='postgres', password='1234', host='localhost', port='5433') 
    cur = conn.cursor()
    cur.execute("""
    ...
    while True: 
        ...
        if not paused: 
            ...
            if new_head == food['pos']: 
                ...
                if score % 3 == 0: 
                    level += 1 
                food = get_random_food()  #для созданий новой еды
            else: 
                snake_pos.pop() 

            #еда каждые 10 секунд исчезновение 
